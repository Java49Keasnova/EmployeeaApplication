...
public class CompanyProtocol implements ApplProtocol {
	...
	private Serializable employees_age(Serializable data) {
		FromTo ageFromTo = (FromTo) data;
		int from = ageFromTo.from();
		int to = ageFromTo.to();
		return new ArrayList<Employee>(company.getEmployeesByAge(from, to));
	}

	private Serializable employees_salary(Serializable data) {
		FromTo salaryFromTo = (FromTo)data;
		int from = salaryFromTo.from();
		int to = salaryFromTo.to();
		return new ArrayList<Employee>(company.getEmployeesBySalary(from, to));
	}

	private Serializable employees_department(Serializable data) {
		String department = (String)data;
		return new ArrayList<Employee>(company.getEmployeesByDepartment(department));
	}

	private Serializable salary_distribution(Serializable data) {
		int interval = (int) data;
		return new ArrayList<SalaryDistribution>(company.getSalaryDistribution(interval));
	}


	private Serializable department_salary_distribution(Serializable data) {
	
		return  new ArrayList<DepartmentSalary>(company.getDepartmentSalaryDistribution());
	}
	...
	Serializable employees_get(Serializable data) {
		...
		return new ArrayList<Employee> (company.getEmployees());
	}
